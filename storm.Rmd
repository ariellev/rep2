...
This report is based on a NOAA Storm Dataset containing over 900K meteorological events made across the U.S. between 1950 and 2011. The meteorolgical events were categorized into 48 different types specified at **Table 1: Storm Data** in [National Weather Service Instruction 10-1605.pdf](https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf) . The focus of my study was to assess the damage made to both property and publich health, categorized by each and every event type. The events with the most average mortality rates were: tsunami (1.65) , heat (1.27), excessive heat (1.10) and rip current (0.73). The study of the correlation between injury and fatality rate is by itself an ineresting question, but unfortunately will not be covered by the scope of this report. Among the most destructive events in terms of loss of property are flood, hurricane/typhoon, and tornados with averagely more than 150, 85 and 57 Billions USD correspondly. In the crops category are drought, flood and hurricane/typhoon the most destructive with an average damage of 13.9, 10.8, 5.5 Billions USD each correspondly.
...
```{r noaa_e_types.txt,echo=T, cache=T}
...
m$EVTYPE <- factor(m$EVTYPE,levels=m$EVTYPE)
ggplot(m, aes(x=EVTYPE, y=total_prop_dmg)) + geom_bar(colour="black", fill = "green", stat="identity", position=position_dodge(), size=.3) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylab("Total") + xlab("Event") + ggtitle(expression(atop("Total property damage in billions of USD", atop("USA 1950 - 2011"))))

# crop damage
m <- summarized_dmg[,c(1,4)]
m <- m[order(-m$total_crop_dmg),]
m$EVTYPE <- factor(m$EVTYPE,levels=m$EVTYPE)
ggplot(m, aes(x=EVTYPE, y=total_crop_dmg)) + geom_bar(colour="black", fill = "green", stat="identity", position=position_dodge(), size=.3) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylab("Total") + xlab("Event") + ggtitle(expression(atop("Total crop damage in billions of USD", atop("USA 1950 - 2011"))))
```
